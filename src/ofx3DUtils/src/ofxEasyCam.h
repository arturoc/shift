...
public:
	...
	void keyPressed(ofKeyEventArgs & event){
		if(event.key==OF_KEY_UP) position(getPosition()+ofxVec3f(0,0,1));
		if(event.key==OF_KEY_DOWN) position(getPosition()+ofxVec3f(0,0,-1));
		if(event.key==OF_KEY_LEFT) position(getPosition()+ofxVec3f(-1,0,0));
		if(event.key==OF_KEY_RIGHT) position(getPosition()+ofxVec3f(1,0,1));
	}
